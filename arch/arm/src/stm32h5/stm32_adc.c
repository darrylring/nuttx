      ...
      if (priv->circular)
        {
          ...
          /* CONT and a hardware trigger are mutually exclusive: with
           * CONT=1 the first edge starts the ADC and it then free-runs,
           * ignoring subsequent triggers.  Keep CONT clear whenever a
           * trigger paces conversions:
           *   - ADC_HAVE_TIMER + tbase!=0: a driver-owned timer triggers.
           *   - ADC_HAVE_EXTCFG: the board selects an external trigger at
           *     runtime via STM32_ADC_EXTCFG_SET (e.g. TIM8 TRGO).
           * Only a free-running circular DMA (no trigger at all) sets CONT.
           */
...
#ifdef ADC_HAVE_EXTCFG
  adc_extcfg_set(priv, priv->extsel);
#endif
...
static int adc_timinit(struct stm32_dev_s *priv)
{
  ...
  /* NOTE: EXTSEL configuration is done in adc_setup() via
   * adc_extcfg_set(priv, priv->extsel).  This function only programs a
   * timer that the ADC driver owns.
   */
